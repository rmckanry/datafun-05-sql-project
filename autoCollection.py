...
sql_file_path = pathlib.Path("sql").joinpath("tableCreation.sql")
makers_data_path = pathlib.Path("data").joinpath("autoMaker.csv")
models_data_path = pathlib.Path("data").joinpath("autoModel.csv")
...
def insert_data_from_csv(db_path, makers_data_path, models_data_path):
    ...
    try:
        ...
        models_df = pd.read_csv(models_data_path)
        with sqlite3.connect(db_path) as conn:
            makers_df.to_sql("makers", conn, if_exists="replace", index=False)
...
def main():
    ...
    insert_data_from_csv(db_file_path, makers_data_path, models_data_path)
